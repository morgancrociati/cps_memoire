...
void *mem_alloc(size_t taille)
{
	...
	if (emplacement->size > actualSize)
	{
		...
		//On supprime l'ancien block libre
		tmp = premierFB;
		//Cas où le bloc libre était le premier block libre
		if (tmp == emplacement)
		{
			/*Comme l'emplacement était le premier bloc libre on ajoute
			celui que l'on vient de créer comme premier bloc libre, et il
			est forcément inférieur en taille à celui d'avant.*/
		...
		else
		{
			...
			//On ajoute le nouveau block libre
			//Notre nouveau emplacement est le plus petit
			if (premierFB->size > prochainEmplacement->size)
			{
				prochainEmplacement->next = premierFB;
				memoire[0] = prochainEmplacement;
			}
			else
			{
				tmp = premierFB;
				while (tmp->next != NULL && tmp->next->size > prochainEmplacement->size)
				{
					tmp = tmp->next;
				}
				//Cela signifie que l'emplacement obtenue est le plus grand
				if (tmp->next == NULL)
				{
					tmp->next = prochainEmplacement;
					prochainEmplacement->next = NULL;
				}
				else
				{
					prochainEmplacement->next = tmp->next;
					tmp->next = prochainEmplacement;
				}
			}
		}
